...
def make_graph(nodes="rrNodes.txt", node_city="rrNodeCity.txt", edges="rrEdges.txt"):
    ...
    for i in file.readlines():
        ...
        nodeLoc[content[0]] = (content[1], content[2])
        neighbors[content[0]] = []
    file.close()

    file2 = open(node_city)
    for i in file2.readlines():
        content = i.strip().split()
        if len(content) == 3:
            nodeToCity[content[0]] = content[1] + " " + content[2]
            cityToNode[content[1] + " " + content[2]] = content[0]
        else:
            nodeToCity[content[0]] = content[1]
            cityToNode[content[1]] = content[0]
    file2.close()

    file3 = open(edges)
    for i in file3.readlines():
        content = i.strip().split()
        neighbors[content[0]].append(content[1])
        neighbors[content[1]].append(content[0])
        y1, x1 = nodeLoc[content[0]][0], nodeLoc[content[0]][1]
        y2, x2 = nodeLoc[content[1]][0], nodeLoc[content[1]][1]
        edgeCost[(content[0], content[1])] = calc_edge_cost(y1, x1, y2, x2)
        edgeCost[(content[1], content[0])] = calc_edge_cost(y2, x2, y1, x1)
    file3.close()
...
def draw_all_edges(ROOT, canvas, graph):
    ...
    for n1, n2 in graph[4]:  # graph[4] keys are edge set
        drawLine(canvas, *graph[5][n1], *graph[5][n2],
                 'white')  # graph[5] is map dict
    ROOT.update()
...
def bi_bfs(start, goal, graph, col):
    ...
    counter = 0
    frontier, explored = deque(), {start: "s"}
    brontier, bexplored = deque(), {goal: "s"}
    frontier.append(start)
    brontier.append(goal)
    while frontier and brontier:
        s = frontier.popleft()
        if s in bexplored:
            path, cost = generate_path(s, explored, graph)
            bath, bost = generate_path(b, bexplored, graph)
            return path+bath[::-1], cost+bost, len(explored)+len(bexplored)
        ...
        for c in graph[3][b]:
            if c not in bexplored:
                bexplored[c] = b
                brontier.append(c)
                drawLine(canvas, *graph[5][b], *graph[5][c], col)
        counter += 1
        if counter % 100 == 0:
            ROOT.update()
    return None
...
def a_star(start, goal, graph, col, heuristic=dist_heuristic):
    ...
    pathcost = 0
    frontier.push((heuristic(start, goal, graph), start, [start], pathcost))
    while not frontier.isEmpty():
        cost, current, path, pathcost = frontier.pop()
        if current == goal:
            draw_final_path(ROOT, canvas, path, graph)
            return path, pathcost, len(explored)
        for a in graph[3][current]:
            f = heuristic(a, goal, graph) + graph[4][(current, a)] + pathcost
            if (a in explored and f < explored[a]) or (a not in explored):
                pathcost += graph[4][(current, a)]
                frontier.push((f, a, path + [a], pathcost))
                explored[a] = f
                drawLine(canvas, *graph[5][current], *graph[5][a], col)
        counter += 1
        if counter % 100 == 0:
            ROOT.update()
    return None
...
def bi_a_star(start, goal, graph, col, heuristic=dist_heuristic):
    ...
    counter = 0
    frontier = PriorityQueue()
    brontier = PriorityQueue()
    x = heuristic(start, goal, graph)
    y = heuristic(goal, start, graph)
    explored = {start: x}
    bexplored = {goal: y}
    pathcost = 0
    frontier.push((x, start, [start], pathcost))
    brontier.push((y, goal, [goal], pathcost))
    frontier.push((heuristic(start, goal, graph), start, [start], pathcost))
    while frontier.isEmpty() == False and brontier.isEmpty() == False:
        cost, current, path, pathcost = frontier.pop()
        bost, burrent, bath, bathcost = brontier.pop()
        for a in graph[3][current]:
            f = heuristic(a, goal, graph) + graph[4][(current, a)] + pathcost
            if (a in explored and f < explored[a]) or (a not in explored):
                pathcost += graph[4][(current, a)]
                ...
                if current in bexplored:
                    draw_final_path(ROOT, canvas, path, graph)
                    return path+bath[::-1], pathcost+bathcost, len(explored)+len(bexplored)
        for b in graph[3][burrent]:
            f = heuristic(b, start, graph) + graph[4][(burrent, b)] + bathcost
            if (b in bexplored and f < bexplored[b]) or (b not in bexplored):
                bathcost += graph[4][(burrent, b)]
                brontier.push((f, b, bath + [b], bathcost))
                bexplored[b] = f
                drawLine(canvas, *graph[5][burrent], *graph[5][b], col)
                if burrent in explored:
                    draw_final_path(ROOT, canvas, bath, graph)
                    return path+bath[::-1], pathcost+bathcost, len(explored)+len(bexplored)
        counter += 1
        if counter % 100 == 0:
            ROOT.update()
...
def tri_directional(city1, city2, city3, graph, col, heuristic=dist_heuristic):
    ...
    draw_all_edges(ROOT, canvas, graph)
    # Your code goes here
    path1, cost1 = a_star(city1, city2, graph, col, heuristic)
    path2, cost2 = a_star(city1, city3, graph, col, heuristic)
    path3, cost3 = a_star(city2, city3, graph, col, heuristic)
    totalcost1 = cost1+cost2
    totalcost2 = cost1+cost3
    totalcost3 = cost2+cost3
    mincost = min(totalcost1, totalcost2, totalcost3)
    if mincost == totalcost1:
        return path1+path2, totalcost1
    if mincost == totalcost2:
        return path1+path3, totalcost2
    if mincost == totalcost3:
        return path2+path3, totalcost3


def main():
    start, goal = input("Start city: "), input(
        "Goal city: ")  # , input("Third city for tri-directional: ")
    graph = make_graph("rrNodes.txt", "rrNodeCity.txt",
                       "rrEdges.txt")  # Task 1

    cur_time = time.time()
    # graph[2] is city to node
    path, cost, l = bfs(graph[2][start], graph[2][goal], graph, 'yellow')
    print(l)
    if path != None:
    ...
    cur_time = time.time()
    path, cost, l = bi_bfs(graph[2][start], graph[2][goal], graph, 'green')
    print(l)
    if path != None:
        display_path(path, graph)
        print("The whole path:", path)
    ...
    cur_time = time.time()
    path, cost, l = a_star(graph[2][start], graph[2][goal], graph, 'blue')
    print(l)
    if path != None:
        display_path(path, graph)
        print("The whole path:", path)
    ...
    cur_time = time.time()
    path, cost, l = bi_a_star(
        graph[2][start], graph[2][goal], graph, 'orange')
    print(l)
    if path != None:
        display_path(path, graph)
        print("The whole path:", path)
   ...
   #  cur_time = time.time()
   #  path, cost = tri_directional(
   #      graph[2][start], graph[2][goal], graph[2][third], graph, 'pink')
   #  if path != None:
   #      display_path(path, graph)
   #      print("The whole path:", path)
   #      print("The length of the whole path:", len(path))
